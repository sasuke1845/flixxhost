...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSY FLIXHOST OFFC             [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "flixhost" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu Gih Ke Flixhost${NC}"
    echo -e "${YELLOW}TELEGRAM : @flixhost${NC}"
    echo -e "${YELLOW}WHATSAPP : 628386045434${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 20K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©FlixhostOffc${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/sasuke1845/flixxhost/raw/main/C2.zip")
        break
        ;;
      2)
        ...
        ;;
      3)
        THEME_URL=$(echo -e "https://github.com/sasuke1845/flixxhost/raw/main/C3.zip")
        break
        ;; 
      x)
        return
...
# Unduh file tema
wget -O /root/C2.zip https://github.com/sasuke1845/flixxhost/raw/main/C2.zip
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer flixxhost   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : flixxhost      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : FLIXHOSTOFFC  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by FlixhostOffc  ${NC}"
